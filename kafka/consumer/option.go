...
func WithKotelTracer(tracer *kotel.Tracer) Option {
...
func WithKgoLatest() Option {
	return consumerOption(func(c config) config {
		c.opts = append(c.opts, kgo.ConsumeResetOffset(kgo.NewOffset().AtStart()))
		return c
	})
}
...
func WithFetchCount(count int) Option {
	return consumerOption(func(c config) config {
		c.fetchCount = count
		return c
	})
...
func WithProcessor(processor contract.Processor) Option {
